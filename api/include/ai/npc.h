...
class Npc {
 public:
  ...
  [[nodiscard]] sf::Vector2i const get_current_position() {
    return {static_cast<int>(motor_.GetPosition().x), static_cast<int>(motor_.GetPosition().y)};
  }
  ...
  [[nodiscard]] PathStatus const TryToGetPathToClosestResource(std::vector<resource::Resource> &resourcesPosition,
                                                               AStarGraph &graph,
                                                               std::span<sf::Vector2i> walkable);
  ...
  [[nodiscard]] ResourcesType const get_wanted_resource(){return wanted_resource_;};

  bool need_path_ = false;
  PathRequest path_request_ = PathRequest::kNone;
  bool brought_back_resource_home = false; //when true, will add the collected resource
 ...
 private:
  ...
  ResourcesType wanted_resource_ = ResourcesType::kNone;
